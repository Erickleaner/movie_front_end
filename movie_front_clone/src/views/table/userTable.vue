...
import { insert, remove, update } from '@/api/article' // secondary package based on el-pagination
import { fetchList } from '@/api/manageUser'
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      ...
      //$nextTick 是Vue.js中的一个实例方法，它用于在下一个DOM更新循环周期中执行回调函数。
      //虚拟DOM更新后立即访问更新后的DOM,这对于需要在操作DOM元素之前确保DOM已经更新的情况非常有用。
      this.$nextTick(() => {
        //这个函数移除表单的校验结果
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          update(tempData).then(res => {
            if (res.data){
              const index = this.list.findIndex(v => v.articleId === this.temp.articleId)
